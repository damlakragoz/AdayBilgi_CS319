    ...
    const handleApprove = async (fairInvId) => {
        try {
            ...
            if (response.status === 200 || response.status === 201 || response.status === 202) {
                ...
                toast.info("Fuar daveti onaylandı!");
            } else {
                console.log("Unexpected response status:" + response.status);
                toast.error("Fuar daveti onaylanamadı. Daha sonra tekrar deneyiniz.");
            }
        } catch (error) {
    ...
    const handleReject = async (fairInvId) => {
        try {
            ...
            if (response.status === 200 || response.status === 201 || response.status === 202) {
                ...
                toast.info("Fuar daveti başarıyla reddedildi!");
            }
        } catch (error) {
            console.error("Error rejecting tour:", error.message);
            toast.error("Fuar daveti reddedilemedi. Lütfen tekrar deneyin.");
        }
    };
